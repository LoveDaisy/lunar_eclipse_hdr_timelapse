function [hist_store, exp_store, types] = test_valid_image(image_path, files)
total_images = length(files);

x_highlight = 94:.1:100;
x_median = 47:.1:53;
hist_store = zeros(total_images, length(x_highlight)+length(x_median));
exp_store = zeros(total_images, 1);
types = nan(total_images, 1);
...
for i = 1:total_images
	...
	y_median = y(1:length(x_median));
    y_highlight = y(1+length(x_median):end);
    
    figure(1); clf;
    set(gcf, 'Position', [300, 250, 800, 400]);
    subplot(1,2,1);
    imshow(img_v_ec);
    title(sprintf('median: %.4f', y(30)));
    subplot(1,2,2)
    plot(x_highlight-min(x_highlight), y_highlight, x_median-min(x_median), y_median,...
        [94.5, 94.5], [0, 1], 'k:', [94.3, 94.3], [0, 1], 'k:', ...
